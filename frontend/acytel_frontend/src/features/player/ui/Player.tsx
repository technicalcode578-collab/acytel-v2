...
export const Player: Component = () => {
    ...
    return (
        <Show when={playerState.currentTrack()}>
            <div class="fixed bottom-0 left-0 right-0 bg-gray-800 text-white p-4 flex items-center justify-between shadow-lg border-t border-gray-700">
                <div class="flex items-center w-1/4">
                    <Show when={playerState.currentTrack()} keyed>
                        {(track) => (
                            <>
                                <img src={`https://placehold.co/64x64/1a202c/ffffff?text=${track.title[0]}`} alt="Album Art" class="w-16 h-16 rounded mr-4" />
                                <div>
